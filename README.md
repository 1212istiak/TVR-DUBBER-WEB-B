...
- **Auth** — simple per-request password check, bcrypt-hashed (via `bcryptjs`, a
...
backend/
  ...
  middleware/             admin password guard + rate limiters
...
```
cd backend
cp .env.example .env        # fill in ADMIN_INITIAL_PASSWORD at minimum — see comments in the file
npm install
...
complete 8-section admin panel, per-request password + bcrypt auth with rate limiting,
